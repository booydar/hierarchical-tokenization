...
for DATA_PATH in "N8-K2V2-V62_1M" "N16-K2V2-V62_1M"; do
  for MEMORY_TASK_FREQ in 0.5 0.25 0.75; do
    ...
    USE_MEM_PROJ=false
    MEM_PROJ_MODE="proj"
    for OUT_LAYER_IDX in -1 -2 -3 -4; do
      for LR in 3e-04 1e-04; do
        for N in 1 2; do
          RUN_NAME=original_rmt-vl_${BASE_MODEL}_L${L}H${H}D${D}-out_layer_${OUT_LAYER_IDX}_mem${N_MEM_TOKENS}_lr${LR}-${MODE}_${MEMORY_TASK_FREQ}
          ...
          run_original_rmt_on_kv_retrieval-vl.py \
            --exp_path $EXP_PATH \
