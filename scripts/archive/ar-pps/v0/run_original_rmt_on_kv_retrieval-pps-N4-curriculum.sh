...
PAIRS_PER_SEGMENT_VALUES=(1 2 4)

for i in "${!PAIRS_PER_SEGMENT_VALUES[@]}"; do
  PAIRS_PER_SEGMENT=${PAIRS_PER_SEGMENT_VALUES[$i]}
  if [ $i -eq 0 ]; then
    PREV_PAIRS_PER_SEGMENT=0
  else
    PREV_PAIRS_PER_SEGMENT=${PAIRS_PER_SEGMENT_VALUES[$((i-1))]}
  fi
  for N in 1; do
    for LR in 1e-04; do
      RUN_NAME=original_rmt_${BASE_MODEL}_L${L}H${H}D${D}_mem${N_MEM_TOKENS}_lr${LR}-pps${PAIRS_PER_SEGMENT}-curr
      PREV_RUN_NAME=original_rmt_${BASE_MODEL}_L${L}H${H}D${D}_mem${N_MEM_TOKENS}_lr${LR}-pps${PREV_PAIRS_PER_SEGMENT}-curr
      ...
      if [ $i -eq 0 ]; then
        ...
        PREV_EXP_PATH="/workspace-SR006.nfs2/bulatov/rmt/runs/test-time-pps/${DATA_PATH}/${PREV_RUN_NAME}/run_$N"
      fi
