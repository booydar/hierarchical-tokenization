...
for N_MEM_TOKENS in 8 32; do
  for LR in 3e-04 1e-04; do
     for N in 1; do
        for PAIRS_PER_SEGMENT in 4 8; do
          N_PAIRS=$((N_SEGMENTS * PAIRS_PER_SEGMENT))
          DATA_PATH="N${N_PAIRS}-K${K}V${V}-V62_1M"

          RUN_NAME=rmca-v2.0-mem-layers_${BASE_MODEL}_L${L}H${H}D${D}_mem${N_MEM_TOKENS}_lr${LR}-${N_SEGMENTS}x${PAIRS_PER_SEGMENT}-${N_MEM_LAYERS}L-${N_MEM_LAYERS}W
