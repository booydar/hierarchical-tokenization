...
DATA_PATHS=("N4-K2V2-V62_1M" "N8-K2V2-V62_1M" "N16-K2V2-V62_1M" "N32-K2V2-V62_1M")
...
for i in "${!DATA_PATHS[@]}"; do
  ...
  if [ $i -eq 0 ]; then
    ...
    PREV_DATA_PATH=${DATA_PATHS[$((i-1))]}
  fi
  # for N in 1 2; do
  for N in 1; do
    for LR in 1e-04; do
      RUN_NAME=original_rmt_${BASE_MODEL}_L${L}H${H}D${D}_mem${N_MEM_TOKENS}_lr${LR}-pps${PAIRS_PER_SEGMENT}-curr_T
      PREV_RUN_NAME=original_rmt_${BASE_MODEL}_L${L}H${H}D${D}_mem${N_MEM_TOKENS}_lr${LR}-pps${PAIRS_PER_SEGMENT}-curr_T
      ...
      EXP_PATH="/workspace-SR006.nfs2/bulatov/rmt/runs/test-time-pps/${DATA_PATH}/${RUN_NAME}/run_$N"
      if [ $i -eq 0 ]; then
        PREV_EXP_PATH=None
      else
        PREV_EXP_PATH="/workspace-SR006.nfs2/bulatov/rmt/runs/test-time-pps/${PREV_DATA_PATH}/${PREV_RUN_NAME}/run_$N"
      fi
