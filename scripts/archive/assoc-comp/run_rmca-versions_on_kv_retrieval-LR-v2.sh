...
for N in 1 2; do
  for PAIRS_PER_SEGMENT in 8 4; do
    for N_MEM_TOKENS in 32; do
      for LR in 1e-06 3e-06 1e-05 3e-05 1e-04 3e-04 1e-03 3e-03; do

        N_PAIRS=$((N_SEGMENTS * PAIRS_PER_SEGMENT))
        DATA_PATH="N${N_PAIRS}-K${K}V${V}-V62_1M"

        RUN_NAME=rmca-v2.0-write-order_${BASE_MODEL}_L${L}H${H}D${D}_mem${N_MEM_TOKENS}_lr${LR}-${N_SEGMENTS}x${PAIRS_PER_SEGMENT}
